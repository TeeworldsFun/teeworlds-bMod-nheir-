...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, int MaxDamage, int64 AffectedMask)
{
	// create the event
	...
	for(int i = 0; i < Num; i++)
	{
		if(g_Config.m_SvSprayprotection && !CmaskIsSet(AffectedMask, apEnts[i]->GetPlayer()->GetCID()))
			continue;
